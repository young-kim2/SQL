...
전체직원의 
다음 정보를 조회하세요.
정렬은 입사일 (hire_date)의 올림차순(ASC)
으로 가장 선임부터 출력이 되도록 하세요
...
문제
2.
업무(jobs)별로 업무이름 job_title 과 최고 월급(max_salary)을
...
/*
문제 3.
담당 매니저가 배정되어있으나 커미션비율이 
없고 월급이 3000 초과인 직원의 이름,매니저 아이디 커미션 비율 월급
을 출력하세요
*/
DESC employees;
SELECT first_name,
...
/*
문제 4.
최고월급(max_salary) 이 10000이상인 
업무의 이름 job_title과 최고월급(max_salary)을
최고월급(max_salary) 내림차순(DESC)로 정렬하여
출력하세요.
*/
SELECT job_title,max_salary
FROM jobs
...
/*
문제 9
정확하지는 않지만, 지사가 있을 것으로 예상되는 나라들을 
나라이름을 대문자로 출력하고 
올림차순(ASC)으로 정렬해 보세요
*/
...
/*
문제 10
입사일이 03/12/31일 이전 입사한
직우너의 이름, 월급, 전화번호, 입사일 을 출력하세요
